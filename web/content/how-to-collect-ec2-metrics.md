...
-`InstanceId`: Isolates data from a specific instance.-`AutoScalingGroupName`: Filters instances by their [Auto Scaling group][autoscaling], if they are assigned to one.-`ImageId`: Filters instances by their [Amazon Machine Image (AMI)][ami]. _This dimension is only available for instances with detailed monitoring enabled._-`InstanceType`: Filters instances by instance type. _This dimension is only available for instances with detailed monitoring enabled._  
### Periods  
...
- browse available metrics for a quick overview of your instances,- create dashboards to track multiple metrics, and- set alarms to notify you if metrics pass fixed thresholds.  
#### Browse metrics  
...
-`metric-name`-`start-time`_([ISO 8601 UTC format][iso])_-`end-time`_(ISO 8601 UTC format)_-`period`_(in seconds)_  
